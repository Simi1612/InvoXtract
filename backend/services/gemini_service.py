...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = os.path.abspath(_SA_PATH)
vertexai.init(project=_PROJECT_ID, location=_LOCATION)

model = GenerativeModel("gemini-2.5-flash")

ALL_FIELDS = [
    "invoice_number", "date", "due_date", "payment_terms", "currency",
    "vendor_name", "vendor_address", "buyer_name", "buyer_address",
...
def _image_from_bytes(file_bytes: bytes, mime_type: str) -> Image.Image:
    if mime_type == "application/pdf":
        return convert_from_bytes(file_bytes, first_page=1, last_page=1, dpi=200, poppler_path=_POPPLER_PATH)[0]
    return Image.open(io.BytesIO(file_bytes))


def _extract_image(pil_img: Image.Image, fields: list[str] | None) -> dict:
    vertex_img = _pil_to_vertex_image(pil_img)
    response = model.generate_content([_build_prompt(fields), vertex_img])
    raw = _clean_json(response.text)
    try:
        return json.loads(raw)
    except json.JSONDecodeError as e:
        raise ValueError(f"Model returned invalid JSON: {str(e)}. Raw: {raw[:200]}")


def extract_invoice(file_bytes: bytes, mime_type: str, fields: list[str] | None = None) -> dict:
    """Extract single invoice (first page if PDF)."""
    return _extract_image(_image_from_bytes(file_bytes, mime_type), fields)


def extract_all_pages(file_bytes: bytes, fields: list[str] | None = None) -> list[dict]:
    """Extract every page of a multi-page PDF as a separate invoice."""
    images = convert_from_bytes(file_bytes, dpi=200, poppler_path=_POPPLER_PATH)
    results = []
    for img in images:
        try:
            results.append(_extract_image(img, fields))
        except Exception as e:
            results.append({"_error": str(e), "_page": len(results) + 1})
    return results
